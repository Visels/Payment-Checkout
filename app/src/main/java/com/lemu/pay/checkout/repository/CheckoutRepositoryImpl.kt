...
class CheckoutRepositoryImpl (
    ...
    override suspend fun initiateSTK(request: CheckoutInitiateSTKRequest): Flow<InitiateSTKResponse> {
        return flow {
    ...
    override suspend fun fetchTransactionStatus(transactionId: String): Flow<TransactionStatusResponse> {
        return flow {
            emit(api.fetchTransactionStatus(transactionId))
        }
    }
}